...
numbers_dir = numbers_dir + "/" + exp_name
if not os.path.exists(numbers_dir):
    os.makedirs(numbers_dir)
...
for i in range(1, 17):
    ...
    config_file = f'./configs/{exp_name}/cmx-{i}.ini'
    verbose = False
    ...
    # Load precomputed fission
    opt_fission = {}
    fission_file = f'./numbers/planaria-hardware-summary-{i}.csv'
    fp = open(fission_file, "r")
    lines = fp.readlines()[1:]
    for line in lines:
        tokens = line.split(',')
        N = int(float(tokens[1]))
        M = int(float(tokens[2]))
        thread = float(tokens[3])
        model_name = tokens[5]
        instr_name = tokens[6]

        if not model_name in opt_fission:
            opt_fission[model_name] = {}
        
        opt_fission[model_name][instr_name] = [N, M, thread]

    cmx_sim.run_solo_mode(opt_fission)
    ...
    final_cmx_df.to_csv(f'./numbers/{exp_name}/planaria-hardware-summary-{cmx_sim.num_active_cores}.csv')